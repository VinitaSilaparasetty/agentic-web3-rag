    ...
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 p-6 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center gap-4 mb-6">
          <Image
            src="/aevoxis-logo.png"
            alt="Aevoxis Solutions"
            width={56}
            height={56}
            priority
          />
          <div>
            <h1 className="text-2xl font-semibold leading-tight">Web3 Docs Search</h1>
            <p className="text-xs text-gray-500 mt-0.5">Powered by Aevoxis Solutions</p>
          </div>
        </div>
        ...
        <ul className="space-y-4">
          {items.map((it, i) => (
            <li key={i} className="border p-4 rounded">
              ...
              {it.project && (
                <div className="text-xs mt-1 opacity-70">project: {it.project}</div>
              )}
      ...
      <footer className="border-t py-4 text-center text-xs text-gray-400">
        © {new Date().getFullYear()} Vinita Silaparasetty · Aevoxis Solutions. All rights reserved.
      </footer>
    </div>
  );
}
