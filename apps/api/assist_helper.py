...
def _special_case_geth_eth_getBalance(q: str, docs: List[Dict]) -> str | None:
    ...
    if ("geth" in ql or "go-ethereum" in ql) and ("eth_getbalance" in ql or "getbalance" in ql):
        ...
        lines.append("1. Start geth with HTTP JSON-RPC enabled (limit to local / trusted networks):")
        lines.append("")
        lines.append("```bash")
        lines.append("geth \\")
        lines.append('  --http \\')
        ...
        lines.append("Use a JSON-RPC POST to `http://127.0.0.1:8545` with a target address and block tag:")
        lines.append("")
        lines.append("```bash")
        lines.append("curl -s http://127.0.0.1:8545 -H 'Content-Type: application/json' -d '")
        lines.append('{"jsonrpc":"2.0","method":"eth_getBalance","params":["0xYourAddressHere","latest"],"id":1}')
        lines.append("'")
        lines.append("```")
        lines.append("")
        lines.append("**Why this works:** `eth_getBalance` returns the account balance (in wei) at the given block tag. Use `latest` for the head of the chain or a hex block number like `0xA`. Convert wei to ETH by dividing by 1e18.")
        # cite sources list
        srcs = []
        seen = set()
        for d in docs:
...
def _fallback_structured_summary(q: str, docs: List[Dict]) -> str:
    ...
    for d in docs:
        u = d.get("url")
        if u and u not in seen:
            seen.add(u); title = d.get("title") or d.get("source") or u
            dom = (d.get("source") or "").strip()
            srcs.append(f"- {title} ({dom}) → {u}")
        if len(srcs) >= 5: break
    if srcs:
