import os, re, sys, time, json, urllib.parse, requests
...
def add(url):
    ...
    if PER_DOMAIN.get(host,0) >= MAX_PER_DOMAIN: return False
    SEEN.add(u)
...
def try_sitemap(root):
    ...
    for path in ("/sitemap.xml", "/sitemap_index.xml"):
        ...
        try:
            r = requests.get(sm, timeout=REQ_TIMEOUT, headers={"User-Agent":"web3-rag-discover/1.0"})
...
def allowed_by_policy(url):
    ...
    if not pol.allowed: return False
    if not robots_allowed(url): return False
    return True
...
def main():
    ...
    for root in roots:
        if not allowed_by_policy(root): continue
        add(root)
        try_sitemap(root)
        html = fetch(root)
        if html: from_html(root, html)
