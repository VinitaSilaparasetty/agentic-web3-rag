...
def gh_search_repos(q: str, sort: str="stars", order: str="desc", per_page: int=50, pages: int=2) -> List[Dict[str, Any]]:
    items: List[Dict[str, Any]] = []
    for page in range(1, pages+1):
        data = gh_get(f"{GITHUB}/search/repositories", params={"q": q, "sort": sort, "order": order, "per_page": per_page, "page": page})
        items += data.get("items", [])
        if len(data.get("items", [])) < per_page: break
    return items

def gh_repo_license(owner: str, repo: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    try:
...
def guess_docs_url(repo: Dict[str, Any]) -> Optional[str]:
    homepage = (repo.get("homepage") or "").strip()
    if homepage and any(t in homepage.lower() for t in ("docs","readthedocs","vercel.app","netlify.app","site","org","dev")):
        return homepage
...
def discover(limit: int) -> List[Dict[str, Any]]:
    ...
    for q in QUERY_SETS:
        ...
        for r in res:
            ...
            seen.add(key); repos.append(r)
            if len(repos) >= limit: return repos
    return repos[:limit]
...
def upsert_source(doc: Dict[str, Any], entry: Dict[str, Any]) -> None:
    ...
    for s in doc["sources"]:
        if key and s.get("repo") == entry.get("repo"): s.update(entry); return
        if key and s.get("url")  == entry.get("url"):  s.update(entry); return
